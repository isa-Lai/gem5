...
int main(int argc, char* argv[])
{
   ...
   /********************** Fill descriptor data structure for loop 1 ***********************/
	...
	// modified srad_v2 --> make all matrices of stride = 4
    ...
    jW[0]    = 0;
    jE[cols-1] = cols-1;
	// modified srad_v2 --> avoid init the matrix (to mae first access is cache miss)
...
/*     random_matrix(I, rows, cols);
	...
	for (iter=0; iter< niter; iter++){
		...
		sum=0; sum2=0;
		// modified srad_v2
		sum=2.3;sum2=9.71; // Found from a random trial
		/*
		for (i=r1; i<=r2; i++) {
            for (j=c1; j<=c2; j++) {
                tmp   = J[i * cols + j];
