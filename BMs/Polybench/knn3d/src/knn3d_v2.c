...
/************************************************************************************/
   ...
   /*vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv*/
    ...
    // position_z
     /*****************************************************************************************/
   /*vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv*/
    int position_zStride = unitSize ;
    MISA_IndirStreamDesc_t   position_z_metaData   = {LOOP1_ID,0 , RESERVED,  (uint64_t)&position_z[0],
                                                position_zStride , 0 , RESERVED   };
    MISA_Desc_t           position_z_InDirStreamDesc ;
    position_z_InDirStreamDesc.type   = INDIR_DTREAM;
    position_z_InDirStreamDesc.valid  = VALID;
    position_z_InDirStreamDesc.active =  ACTIVE ;
    position_z_InDirStreamDesc.descInfo.stream=position_z_metaData;
    unsigned long long * position_zDesc = (unsigned long long * ) &position_z_InDirStreamDesc;



    // force_z
    MISA_DirStreamDesc_t  force_z_metaData      = { LOOP1_ID , 0 , RESERVED , (uint64_t)&force_z[0],
											     unitSize , 0 , RESERVED };
    MISA_Desc_t           force_z_DirStreamDesc; // = { DIR_STREAM , VALID , ACTIVE  ,arrA_metaData};
    force_z_DirStreamDesc.type   = DIR_STREAM;
    force_z_DirStreamDesc.valid  = VALID;
    force_z_DirStreamDesc.active =  ACTIVE ;
    force_z_DirStreamDesc.descInfo.streamDesc=force_z_metaData;
    // Copy from array descriptor struct to CSRs
    unsigned long long * force_zDesc = (unsigned long long * ) &force_z_DirStreamDesc;
...
printf("&position_z[0] = %p , &position_z[atmos-1] = %p \n",&position_z[0],&position_z[nfAtoms-1]);
printf("&force_z[0]    = %p , &force_z   [atmos-1] = %p \n",&force_z[0],&force_z[nfAtoms-1]);
printf("&NL[0]    = %p , &NL[ngeihbours*atmos-1] = %p \n",&NL[0],&NL[maxfNeighbors*nfAtoms-1]);
    ...
    write_csr(0x806, *(position_xDesc)  ); //  position x
    write_csr(0x807, *(position_xDesc+1)); //

    write_csr(0x808, *(force_xDesc)  ); //  force x
    write_csr(0x809, *(force_xDesc+1)); //

    write_csr(0x80A, *(position_yDesc)  ); //  position y
    write_csr(0x80B, *(position_yDesc+1)); //

    write_csr(0x80C, *(force_yDesc)  ); //  force y
    write_csr(0x80D, *(force_yDesc+1)); //


    write_csr(0x80E, *(position_zDesc)  ); //  position z
    write_csr(0x80F, *(position_zDesc+1)); //

    write_csr(0x810, *(force_zDesc)  ); //  force z
    write_csr(0x811, *(force_zDesc+1)); //
