...
public:
    ...
    void update(typename T::Command cmd, const int* addr, long clk, bool debug_enable=false);
    // Update statistics:

    // Update the number of requests it serves currently
    void update_serving_requests(long Fulladdr,const int* addr, int delta, long clk);
...
private:
    ...
    void update_timing(typename T::Command cmd, const int* addr, long clk,bool update_timing);
}; /* class DRAM */
...
void DRAM<T>::regStats(const std::string& identifier) {
    ...
    refresh_cycles
        ...
        .precision(0)
        .flags(nozero) // gem5::Stats::nozero or Stats::nozero  depending on INTEGRATED_WITH_GEM5 or not
        ;
    busy_cycles
    ...
    active_refresh_overlap_cycles
        ...
        .precision(0)
        .flags(nozero) // gem5::Stats::nozero or Stats::nozero  depending on INTEGRATED_WITH_GEM5 or no
        ;
    serving_requests
...
// Abotaleb: Is this function correct with DDR4 ?
template <typename T>
bool DRAM<T>::check_iteratively(typename T::Command cmd, const int* addr, long clk) {
    ...
    auto cur_node = this;
    // Debug_Detailed [REQ_READY]
    /*if(clk==25926)
    {
        printf("\t\t(Lvl:%d ; child_id=%d ; nextT:%ld) ",int(level),addr[int(level)+1],next[int(cmd)]);
    }*/
    ...
    while(!(child_id < 0 || cur_level == int(spec->scope[int(cmd)]) || (cur_level == (int(T::Level::Row) - 1)))){
        ...
        /*if(clk==25926)
        {
            printf("\t\t(Lvl:%d ; child_id=%d ; cur_node->nextT:%ld) ",int(level),addr[int(level)+1],cur_node->next[int(cmd)]);
            printf("int(spec->scope[int(cmd)]=%d",int(spec->scope[int(cmd)]));
            printf("(int(T::Level::Row) - 1)=%ld\n",(int(T::Level::Row) - 1));        
    ...
    /*if(clk==25926)
    {
        ...
        printf("spec->is_closing(cmd)=%ld\n",spec->is_closing(cmd));
        printf("cur_node->just_opened=%ld\n",cur_node->just_opened);
        printf("cur_level(%d) == int(T::Level::Bank)(%ld)\n",cur_level,int(T::Level::Bank));
        printf("spec->is_closing(cmd)=%ld\n",spec->is_closing(cmd));
        printf("cur_node->just_opened=%ld\n",cur_node->just_opened);
        printf("cur_level(%d) == int(T::Level::Bank)(%ld)\n",cur_level,int(T::Level::Bank));
        printf("cur_level(%d) == int(T::Level::Bank)+1(%ld)\n",cur_level,int(T::Level::Bank)+1);             
...
// Update
template <typename T>
void DRAM<T>::update(typename T::Command cmd, const int* addr, long clk,bool debug_enable)
{
    cur_clk = clk;
    update_state(cmd, addr);
    update_timing(cmd, addr, clk,debug_enable);
}
...
// Update (Timing)
template <typename T>
void DRAM<T>::update_timing(typename T::Command cmd, const int* addr, long clk,bool debug_enable)
{
    // I am not a target node: I am merely one of its siblings
    if (id != addr[int(level)]) {
        /*if(debug_enable)
        {
            printf("id=%d ; addr[int(level)=%d]=%d ",id,int(level),addr[int(level)]);
        }*/
        for (auto& t : timing[int(cmd)]) {
            ...
            /*if(debug_enable)
            {
                print("\tt.cmd=%d ; next[int(t.cmd)]=%ld ; t.val=%d ; clk=%ld ; future = %ld ; ",int(t.cmd),next[int(t.cmd)],t.val,clk,future);
            }*/
            next[int(t.cmd)] = max(next[int(t.cmd)], future); // update future
            /*if(debug_enable)
            {
                print("new next[int(t.cmd)]=%ld\n",next[int(t.cmd)]);
            }*/
        }
    ...
    for (auto& t : timing[int(cmd)]) {
        ...
        /*if(debug_enable)
        {
            print("\tt.cmd=%d ; next[int(t.cmd)]=%ld ; t.val=%d ; clk=%ld ; past=%ld; future = %ld ; ",int(t.cmd),next[int(t.cmd)],t.val,clk,past,future);
        }*/
        next[int(t.cmd)] = max(next[int(t.cmd)], future); // update future
        /*if(debug_enable)
        {
    ...
    for (auto child : children)
        child->update_timing(cmd, addr, clk,debug_enable);

}

template <typename T>
void DRAM<T>::update_serving_requests(long Fulladdr,const int* addr, int delta, long clk) {
  ...
  if (begin_of_cur_reqcnt != -1 && cur_serving_requests > 0) {
    ...
    /*printf("(T:%ld)(A:%lx) active_cycles[Level=%d] = %f cur_serving_requests=%d\n",
    clk, Fulladdr , int(level) , active_cycles.value(),cur_serving_requests);*/
  }
  ...
  /*printf("cur_serving_requests (A: %#lx) (AC: %x %x %x %x %x %x ) (T: %ld) = %d in level = %d and ID = %d \n",
  Fulladdr ,   addr[0],addr[1],addr[2],addr[3],addr[4],addr[5],
  clk,cur_serving_requests,level,id);*/
  ...
  children[child_id]->update_serving_requests(Fulladdr,addr, delta, clk);
}
