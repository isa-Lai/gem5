...
private:
    ...
    int numberCores;
    vector<int> _rank;
    unsigned _markedLoad; //Abotaleb : Sum of marked load for all cores , banks 
...
template<typename T>
void PARBS<T>::dequeue_req(Request req){
    int p = req.coreid; 
    int bcount = req.addr_vec[int(T::Level::Bank)];
    if (ctrl->channel->spec->standard_name == "DDR4" || ctrl->channel->spec->standard_name == "GDDR5")
        bcount += req.addr_vec[int(T::Level::Bank) - 1] * ctrl->channel->spec->org_entry.count[int(T::Level::Bank)];                 				 
    list<Request> q = _markableQ[p][bcount];
    bool found = false;
    for (list<Request>::iterator it=q.begin(); it != q.end(); ++it){
        if(req.addr==it->addr)
            found = true; 
    }
    if(!found)//old if(req.marked)
        return;

    if(_markedLoad > 0)
        _markedLoad--;
	//printf("\tPARBS::Decrease MarkedQ=%d\n",_markedLoad);
...
void PARBS<T>::form_batch(){
	...
	//printf("PARBS::Form Batch:Isolate _markableQ per core per bank\n"); 
    unsigned bcount = 0;
    for (list<Request>::iterator it=ctrl->readq.q.begin(); it != ctrl->readq.q.end(); ++it){
        if(!(it->marked)){
			//Abotaleb : bcount is the actual bank accessed by the requrest 
			int bcount = it->addr_vec[int(T::Level::Bank)];
            if (ctrl->channel->spec->standard_name == "DDR4" || ctrl->channel->spec->standard_name == "GDDR5")
                bcount += it->addr_vec[int(T::Level::Bank) - 1] * ctrl->channel->spec->org_entry.count[int(T::Level::Bank)];                 				 
            int p = it->coreid;
            if((p < numberCores) && (bcount < LocalBcount))
                _markableQ[p][bcount].push_back((*it));
            else
            {
                //printf(" p =%d , numberCores=%d , bcount=%d ,LocalBcount=%d\n",p,numberCores,bcount, LocalBcount);
   ...
   // printf("PARBS::Form Batch\n");   
    ...
    for(int i=0; i < numberCores; i++){
        for(int j=0; j < LocalBcount; j++){
			...
			_markedTotalLoadPerProc[i] += markedCnt;
            if(markedCnt > _markedMaxLoadPerProc[i])
                _markedMaxLoadPerProc[i] = markedCnt;
			/*printf("\t\tM(C:%d)(B:%d)=%d  ;",i,j,markedCnt);
            if(j>0 && j%4==0)
                printf("\n");*/
        }
		//printf("\tMax_M(C:%d)=%d \n",i,_markedMaxLoadPerProc[i]);
		//printf("\tTot_M(C:%d)=%d \n",i,_markedTotalLoadPerProc[i]);
...
void PARBS<T>::assign_rank(){
   ...
   printf("PARBS::Assign ranks:");   
   for(int i=0; i < numberCores; i++)
   {
       _rank[i] = indexof(tids, i, numberCores);
	   printf("\tRank[%d]=%d ",i,_rank[i]);
	   if(i==numberCores-1)
		   printf(";\n");
	   else 
		   printf(", ");
