...
public:
    ...
    static Memory<T> *populate_memory(const Config& configs,std::string cmdTracePath, T *spec, int channels, int ranks) {
        ...
        for (int c = 0; c < channels; c++){
            ...
            channel->id = c;
            channel->regStats("");
            ctrls.push_back(new Controller<T>(configs,cmdTracePath, channel));
        }
        // (InterStellarMC) To support inter-channel comunication for InteRStellar/InterStellarRT [However the implementation can generate the batch in Nucleus]
        for (auto ctrl : ctrls) {
            ctrl->ctrls = ctrls;
        }
    ...
    static MemoryBase *create(const Config& configs,std::string cmdTracePath, int cacheline)
    {
        ...
        return (MemoryBase *)populate_memory(configs,cmdTracePath, spec, channels, ranks);
    }
};

template <>
MemoryBase *MemoryFactory<WideIO2>::create(const Config& configs, std::string cmdTracePath ,int cacheline);
template <>
MemoryBase *MemoryFactory<SALP>::create(const Config& configs,std::string cmdTracePath, int cacheline);
