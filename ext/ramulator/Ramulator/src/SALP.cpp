...
//    {"SALP_512Mb_x4", SALP::Org::SALP_512Mb_x4}, {"SALP_512Mb_x8", SALP::Org::SALP_512Mb_x8}, {"SALP_512Mb_x16", SALP::Org::SALP_512Mb_x16},
 ...
 //   {"SALP_8Gb_x4", SALP::Org::SALP_8Gb_x4}, {"SALP_8Gb_x8", SALP::Org::SALP_8Gb_x8}, {"SALP_8Gb_x16", SALP::Org::SALP_8Gb_x16},
    {"SALP_16Gb_x16CR", SALP::Org::SALP_16Gb_x16CR}
};
...
//    {"SALP_3200", SALP::Speed::SALP_3200},
      {"SALP_3200CR", SALP::Speed::SALP_3200CR}
};
...
SALP::SALP(Org org, Speed speed, Type type, int n_sa) :
    ...
    read_latency(speed_entry.nCL + speed_entry.nBL + speed_entry.nDQSCK)
{
    switch(int(type)){
...
SALP::SALP(const Config& configs, const string& type_str) : SALP(configs["org"], 
        configs["speed"], type_str, configs.get_int("subarrays")){
...
void SALP::set_subarray_number(int sa) {
    int cur_sa = org_entry.count[int(Level::SubArray)];
    int cur_rows = org_entry.count[int(Level::Row)];
    int rows_in_bank = cur_sa*cur_rows;

    n_sa = sa;
    assert(n_sa && n_sa <= 1024 && (n_sa & (n_sa-1)) == 0); // is power of 2, within [1, 128]
    org_entry.count[int(Level::SubArray)] = n_sa;
    org_entry.count[int(Level::Row)] = rows_in_bank/n_sa;
}
...
//        case 3200: speed_entry.nRRD = (page==1) ? 6 : 9; speed_entry.nFAW = (page==1) ? 34 : 48; break;
//        case 3200: speed_entry.nRRD = 
...
//    }
 ...
 //       case 3200: speed_entry.nRFC = (chip==512) ? 144  : (chip==1<<10) ? 177 : (chip==1<<11) ? 266 : (chip==1<<12) ? 417 : 561; break;
 //       default: assert(false);
 //   }
 //   switch (speed_entry.rate) {
 ...
 //       case 3200: speed_entry.nXS  = (chip==512) ? 107 : (chip==1<<10) ? 128 : (chip==1<<11) ? 182 : (chip==1<<12) ? 288 : 384; break;
 //       default: assert(false);
 //   }
    ...
    speed_entry.nREFI = REFI_TABLE[0 /*int(refresh_mode)*/][speed];
...
void SALP::init_lambda()
{
    switch(int(type)){
        case int(Type::SALP_1):
            ...
            lambda[int(Level::Rank)][int(Command::PREA)] = [] (DRAM<SALP>* node, int id) {
                for (auto bank : node->children) {
        ...
        case int(Type::MASA):
            ...
            lambda[int(Level::SubArray)][int(Command::SASEL)] = [] (DRAM<SALP>* node, int id) {
                ...
                for (auto sa : node->parent->children)
                    if (sa != node && sa->state == State::Selected) {
                        ...
                        break;}};
            lambda[int(Level::SubArray)][int(Command::PRE)] = [] (DRAM<SALP>* node, int id) {
                assert(node->state == State::Opened || node->state == State::Selected);
                node->state = State::Closed;
            ...
            lambda[int(Level::Rank)][int(Command::PREA)] = [] (DRAM<SALP>* node, int id) {
                for (auto bank : node->children)
                    for (auto sa : bank->children) {
                        if(sa->state == State::Opened || node->state == State::Selected)
                            sa->collect_opened_cycles(node->cur_clk); // Hasan
                        sa->state = State::Closed;
                        sa->row_state.clear();}};
            
            lambda[int(Level::SubArray)][int(Command::RD)] = [] (DRAM<SALP>* node, int id) {
                node->just_opened = false; // Hasan
            };
            lambda[int(Level::SubArray)][int(Command::WR)] = [] (DRAM<SALP>* node, int id) {
                node->just_opened = false; // Hasan
            };
...
void SALP::init_timing()
{
    ...
    // RAS <-> REF
    t[int(Command::PRE)].push_back({Command::REF, 1, s.nRPSSA});
    t[int(Command::PREA)].push_back({Command::REF, 1, s.nRPab});
    t[int(Command::PRE_OTHER)].push_back({Command::REF, 1, s.nRPSSA});
    ...
    t[int(Command::SRX)].push_back({Command::ACT, 1, s.nXSR});
    ...
    // REF <-> SR
    t[int(Command::SRX)].push_back({Command::REF, 1, s.nXSR});
    ...
    t[int(Command::SRX)].push_back({Command::PDE, 1, s.nXSR});

    // SR <-> SR
    t[int(Command::SRE)].push_back({Command::SRX, 1, s.nSR});
    t[int(Command::SRX)].push_back({Command::SRE, 1, s.nXSR});
    ...
    switch(int(type)) {
        ...
        case int(Type::SALP_2):
        case int(Type::MASA):
          t[int(Command::SASEL)].push_back({Command::SASEL, 1, s.nSCD});
          t[int(Command::SASEL)].push_back({Command::PRE, 1, s.nSCD});
          t[int(Command::SASEL)].push_back({Command::PRE_OTHER, 1, s.nSCD});
          t[int(Command::SASEL)].push_back({Command::PREA, 1, s.nSCD});
        break;
        default: assert(false);
    }
    ...
    t[int(Command::RDA)].push_back({Command::ACT, 1, s.nRTP + s.nRPSSA});
    t[int(Command::WRA)].push_back({Command::ACT, 1, s.nCWL + s.nBL + s.nWR + s.nRPSSA});

    // RAS <-> RAS
    t[int(Command::ACT)].push_back({Command::ACT, 1, s.nRC});
    t[int(Command::ACT)].push_back({Command::PRE, 1, s.nRAS});
    t[int(Command::PRE)].push_back({Command::ACT, 1, s.nRPSSA});
    ...
    switch(int(type)) {
        ...
        case int(Type::MASA):
          ...
          //t[int(Command::SASEL)].push_back({Command::SASEL, 1, s.nSCD+1});
        break;
        default: assert(false);
    }
    ...
    switch(int(type)) {
        case int(Type::SALP_1):
          t[int(Command::PRE)].push_back({Command::ACT, 1, s.nRPDSA, true});
          t[int(Command::PRE_OTHER)].push_back({Command::ACT, 1, s.nRPDSA, true});
          // for auto precharge command
          t[int(Command::ACT)].push_back({Command::ACT, 1, s.nRC - s.nRP + s.nRPDSA, true});
          t[int(Command::RDA)].push_back({Command::ACT, 1, s.nRTP + s.nRPDSA, true});
          t[int(Command::WRA)].push_back({Command::ACT, 1, s.nCWL + s.nBL + s.nWR + s.nRPDSA, true});
        break;

        case int(Type::SALP_2):
          t[int(Command::ACT)].push_back({Command::ACT, 1, s.nRCD + s.nCL/2, true});
          t[int(Command::RD)].push_back({Command::ACT, 1, s.nCL/2, true});
          t[int(Command::RDA)].push_back({Command::ACT, 1, s.nCL/2, true});
          t[int(Command::WR)].push_back({Command::ACT, 1, s.nCWL + s.nBL + s.nWR/2, true});
          t[int(Command::WRA)].push_back({Command::ACT, 1, s.nCWL + s.nBL + s.nWR/2, true});
        break;
        case int(Type::MASA):
          t[int(Command::RD)].push_back({Command::ACT, 1, s.nCL/2, true});
          t[int(Command::RDA)].push_back({Command::ACT, 1, s.nCL/2, true});
          ...
          t[int(Command::RD)].push_back({Command::SASEL, 1, s.nCL/2, true});
          t[int(Command::RDA)].push_back({Command::SASEL, 1, s.nCL/2, true});
          t[int(Command::WR)].push_back({Command::SASEL, 1, s.nCWL + s.nBL + s.nWR/2, true});
          t[int(Command::WRA)].push_back({Command::SASEL, 1, s.nCWL + s.nBL + s.nWR/2, true});

          t[int(Command::RD)].push_back({Command::RD, 1, s.nCL/2, true});
          t[int(Command::RDA)].push_back({Command::RDA, 1, s.nCL/2, true});
          t[int(Command::WR)].push_back({Command::WR, 1, s.nCWL + s.nBL + s.nWR/2, true});
          t[int(Command::WRA)].push_back({Command::WRA, 1, s.nCWL + s.nBL + s.nWR/2, true});
        break;
        default: assert(false);
    }
